...
function renderTemplateCard(template, table, label) {
  ...
  if (table === "diploma_templates") {
    ...
    return `<article class="catalog-card compact-catalog-card"><h3>${escapeHtml(template.name)}</h3><div class="actions">${canvaAction}<button class="btn btn-danger" data-delete-template="${template.id}" data-table="${table}">Eliminar</button></div></article>`;
  }
