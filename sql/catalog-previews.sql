...
create or replace function select_catalog_option_by_token(token text, option_table text, option_id uuid, client_notes text default null)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if option_table = 'package_images' and item.item_type = 'PHOTO_PACKAGE' then
    select pi.package_id, p.name
    into selected_package, selected_name
    from package_images pi
    join packages p on p.id = pi.package_id
...
grant execute on function select_catalog_option_by_token(text, text, uuid, text) to anon, authenticated;
